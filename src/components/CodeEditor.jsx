  ...
  const editorToolbarPanel = useMemo(() => {
    const createPanel = (view) => {
      ...
      const updatePanel = () => {
        ...
        if (hasDiff) {
          toolbarHTML += `
            <span style="font-weight: 500;">${chunkCount > 0 ? `${currentIndex + 1}/${chunkCount}` : '0'} ${t('codeEditor.changes')}</span>
            <button class="cm-diff-nav-btn cm-diff-nav-prev" title="${t('codeEditor.previousChange')}" ${chunkCount === 0 ? 'disabled' : ''}>
              <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
              </svg>
            </button>
            <button class="cm-diff-nav-btn cm-diff-nav-next" title="${t('codeEditor.nextChange')}" ${chunkCount === 0 ? 'disabled' : ''}>
              <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        if (file.diffInfo) {
          toolbarHTML += `
            <button class="cm-toolbar-btn cm-toggle-diff-btn" title="${showDiff ? t('codeEditor.hideDiff') : t('codeEditor.showDiff')}">
              <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ${showDiff ?
        ...
        // Settings button
        toolbarHTML += `
          <button class="cm-toolbar-btn cm-settings-btn" title="${t('codeEditor.editorSettings')}">
            <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        if (isSidebar && onToggleExpand) {
          toolbarHTML += `
            <button class="cm-toolbar-btn cm-expand-btn" title="${isExpanded ? t('codeEditor.collapseEditor') : t('codeEditor.expandEditor')}">
              <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ${isExpanded ?
        ...
        {isSidebar ? (
          <div className="w-full h-full flex items-center justify-center bg-background">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-900 dark:text-white">{t('codeEditor.loading', { fileName: file.name })}</span>
            </div>
          </div>
        ) : (
            ...
            >
              {saveSuccess ? (
                ...
                <>
                  ...
                  <span className="hidden sm:inline">{saving ? t('codeEditor.saving') : t('codeEditor.save')}</span>
                </>
              )}
            </button>
