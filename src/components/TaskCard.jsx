...
const TaskCard = ({
    ...
    switch (status) {
      case 'done':
        return {
          ...
          statusText: t('taskCard.status.done')
        };
      ...
      case 'in-progress':
        return {
          ...
          statusText: t('taskCard.status.inProgress')
        };
      ...
      case 'review':
        return {
          ...
          statusText: t('taskCard.status.review')
        };
      ...
      case 'deferred':
        return {
          ...
          statusText: t('taskCard.status.deferred')
        };
      ...
      case 'cancelled':
        return {
          ...
          statusText: t('taskCard.status.cancelled')
        };
      ...
      default:
        return {
          ...
          statusText: t('taskCard.status.pending')
        };
    }
  };
  ...
  const getPriorityIcon = (priority) => {
    switch (priority) {
      case 'high':
        return (
          <Tooltip content={t('taskCard.priority.high')}>
      ...
      case 'medium':
        return (
          <Tooltip content={t('taskCard.priority.medium')}>
      ...
      case 'low':
        return (
          <Tooltip content={t('taskCard.priority.low')}>
      ...
      default:
        return (
          <Tooltip content={t('taskCard.priority.notSet')}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center">
          {task.dependencies && Array.isArray(task.dependencies) && task.dependencies.length > 0 && (
            <Tooltip content={`${t('taskCard.dependsOn')}: ${task.dependencies.map(dep => `${t('taskCard.task')} ${dep}`).join(', ')}`}>
              <div className="flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400">
                <ArrowRight className="w-3 h-3" />
                <span>{t('taskCard.dependsOn')}: {task.dependencies.join(', ')}</span>
              </div>
            </Tooltip>
          )}
        </div>
        ...
        {/* Status Badge */}
        <Tooltip content={`${t('taskCard.statusLabel')}: ${config.statusText}`}>
      ...
      {task.subtasks && task.subtasks.length > 0 && (
        <div className="ml-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Tooltip content={t('taskCard.subtasksCompleted', { done: task.subtasks.filter(st => st.status === 'done').length, total: task.subtasks.length })}>
            ...
            <Tooltip content={t('taskCard.subtasksSummary', { done: task.subtasks.filter(st => st.status === 'done').length, pending: task.subtasks.filter(st => st.status === 'pending').length, inProgress: task.subtasks.filter(st => st.status === 'in-progress').length })}>
