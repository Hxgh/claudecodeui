...
function ApiKeysSettings() {
  ...
  const deleteApiKey = async (keyId) => {
    if (!confirm(t('apiKeysSettings.confirmDeleteApiKey'))) return;
  ...
  const deleteGithubToken = async (tokenId) => {
    if (!confirm(t('apiKeysSettings.confirmDeleteToken'))) return;
    ...
    <div className="space-y-8">
      ...
      {newlyCreatedKey && (
        <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
          <h4 className="font-semibold text-yellow-500 mb-2">{t('apiKeysSettings.saveApiKey')}</h4>
          <p className="text-sm text-muted-foreground mb-3">
            {t('apiKeysSettings.saveOnlyTime')}
          </p>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">{t('apiKeysSettings.apiKeys')}</h3>
          </div>
          <Button
            size="sm"
          ...
          >
            ...
            {t('apiKeysSettings.newApiKey')}
          </Button>
        </div>

        <p className="text-sm text-muted-foreground mb-4">
          {t('apiKeysSettings.apiKeysDesc')}
        </p>

        {showNewKeyForm && (
          <div className="mb-4 p-4 border rounded-lg bg-card">
            <Input
              placeholder={t('apiKeysSettings.apiKeyNamePlaceholder')}
              value={newKeyName}
              onChange={(e) => setNewKeyName(e.target.value)}
              className="mb-2"
            />
            <div className="flex gap-2">
              <Button onClick={createApiKey}>{t('common.create')}</Button>
        ...
        <div className="space-y-2">
          {apiKeys.length === 0 ? (
            <p className="text-sm text-muted-foreground italic">{t('apiKeysSettings.noApiKeysYet')}</p>
          ) : (
            apiKeys.map((key) => (
              <div
                key={key.id}
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {t('apiKeysSettings.created')}: {new Date(key.created_at).toLocaleDateString()}
                    {key.last_used && ` • ${t('apiKeysSettings.lastUsed')}: ${new Date(key.last_used).toLocaleDateString()}`}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">{t('apiKeysSettings.githubTokens')}</h3>
          </div>
          <Button
            size="sm"
          ...
          >
            <Plus className="h-4 w-4 mr-1" />
            {t('apiKeysSettings.addToken')}
          </Button>
        </div>

        <p className="text-sm text-muted-foreground mb-4">
          {t('apiKeysSettings.githubTokensDesc')}
        </p>

        {showNewTokenForm && (
          <div className="mb-4 p-4 border rounded-lg bg-card">
            <Input
              placeholder={t('apiKeysSettings.tokenNamePlaceholder')}
              value={newTokenName}
            ...
            <div className="relative">
              <Input
                ...
                placeholder={t('apiKeysSettings.githubTokenPlaceholder')}
            ...
            <div className="flex gap-2">
              <Button onClick={createGithubToken}>{t('apiKeysSettings.addToken')}</Button>
        ...
        <div className="space-y-2">
          {githubTokens.length === 0 ? (
            <p className="text-sm text-muted-foreground italic">{t('apiKeysSettings.noGithubTokensYet')}</p>
          ) : (
            githubTokens.map((token) => (
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {t('apiKeysSettings.added')}: {new Date(token.created_at).toLocaleDateString()}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                  ...
                  >
                    {token.is_active ? t('common.active') : t('common.inactive')}
                  </Button>
                  <Button
                    size="sm"
                    variant="ghost"
      ...
      <div className="p-4 bg-muted/50 rounded-lg">
        <h4 className="font-semibold mb-2">{t('apiKeysSettings.externalApiDoc')}</h4>
        <p className="text-sm text-muted-foreground mb-3">
          {t('apiKeysSettings.externalApiDocDesc')}
        </p>
        <a
