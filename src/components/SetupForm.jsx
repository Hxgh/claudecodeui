...
const SetupForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      setError(t('setupForm.passwordMismatch'));
      return;
    }

    if (username.length < 3) {
      setError(t('setupForm.usernameTooShort'));
      return;
    }

    if (password.length < 6) {
      setError(t('setupForm.passwordTooShort'));
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={t('setupForm.usernamePlaceholder')}
                required
                disabled={isLoading}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder={t('setupForm.passwordPlaceholder')}
                required
                disabled={isLoading}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder={t('setupForm.confirmPasswordPlaceholder')}
                required
                disabled={isLoading}
              />
            </div>
