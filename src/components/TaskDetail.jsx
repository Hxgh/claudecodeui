...
const TaskDetail = ({ 
  ...
  const statusOptions = [
    { value: 'pending', label: t('taskDetail.status.pending') },
    { value: 'in-progress', label: t('taskDetail.status.inProgress') },
    { value: 'review', label: t('taskDetail.status.review') },
    { value: 'done', label: t('taskDetail.status.done') },
    { value: 'deferred', label: t('taskDetail.status.deferred') },
    { value: 'cancelled', label: t('taskDetail.status.cancelled') }
  ];
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1">
                <button
                  ...
                  title={t('taskDetail.copyTaskId')}
                >
                  <span>{t('taskDetail.task')} {task.id}</span>
                  <Copy className="w-3 h-3" />
                </button>
                {task.parentId && (
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    {t('taskDetail.subtaskOf', { id: task.parentId })}
                  </span>
                )}
              </div>
              {editMode ? (
            ...
            {editMode ? (
              <>
                <button
                  ...
                  title={isSaving ? t('taskDetail.saving') : t('taskDetail.saveChanges')}
                >
                ...
                <button
                  ...
                  title={t('taskDetail.cancelEditing')}
                >
                  <X className="w-5 h-5" />
                </button>
              </>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 min-h-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('taskDetail.statusLabel')}</label>
              <div className={cn(
            ...
            {/* Priority */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('taskDetail.priorityLabel')}</label>
              <div className={cn(
                'px-3 py-2 rounded-md text-sm font-medium capitalize',
                getPriorityColor(task.priority)
              )}>
                <Flag className="w-4 h-4 inline mr-2" />
                {task.priority || t('taskDetail.notSet')}
              </div>
            </div>

            {/* Dependencies */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('taskDetail.dependenciesLabel')}</label>
              {task.dependencies && task.dependencies.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {task.dependencies.map(depId => (
                    <button
                      ...
                      title={onTaskClick ? t('taskDetail.clickToViewTask', { id: depId }) : `${t('taskDetail.task')} ${depId}`}
                    >
                ...
                <span className="text-gray-500 dark:text-gray-400 text-sm">{t('taskDetail.noDependencies')}</span>
              )}
            </div>
          </div>
          ...
          {/* Description */}
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('taskDetail.descriptionLabel')}</label>
            {editMode ? (
              <textarea
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg">
              ...
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {t('taskDetail.implementationDetails')}
                </span>
                {showDetails ? (
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 p-4">
                  {editMode ? (
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                      placeholder={t('taskDetail.implementationDetailsPlaceholder')}
                    />
                  ) : (
